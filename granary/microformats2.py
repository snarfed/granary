...
def to_as1(mf2, actor=None, fetch_mf2=False, rel_urls=None):
  ...
  for quote in mf2.get('children', []) + props.get('quotation-of', []):
    if isinstance(quote, dict) and 'h-cite' in set(quote.get('type', [])):
      as1_quote = json_to_object(quote)
      is_quotation_of = ('u-quotation-of' in set(quote.get('type', []))
                         or quote in props.get('quotation-of', []))
      if is_quotation_of and as1_quote.get('objectType') != 'note':
        as1_quote['objectType'] = 'note'
      attachments.append(as1_quote)
