...
If you want to work on [oauth-dropins](https://github.com/snarfed/oauth-dropins) at the same time, install it in "source" mode with `pip install -e <path to oauth-dropins repo>`. You'll also need to update the `oauth_dropins` symlink, which is needed for serving static file assets in dev_appserver: `ln -sf <path-to-oauth-dropins-repo>/oauth_dropins`.  To test it with dev_appserver you also need to replace the line in requirements.txt from `git+https://github.com/snarfed/oauth-dropins.git@master#egg=oauth_dropins` to `<path-to-oauth-dropins-repo>` (an alternative workaround for dev_appserver.py can be found [here](https://issuetracker.google.com/issues/144150446).
...
* Standardize Instagram's and Facebook's scraping into new common `Source.scraped_to_activities()` and `Source.scraped_to_reactions()`  methods.
* Flickr:
